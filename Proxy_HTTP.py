# Bản QuyềNgThanhVinhNgThanhVinh
...
def proxyget(url): # scarica proxy da altri siti
	try:
		...
		print("  Đã xảy ra lỗi, bỏ qua trang web tiếp theo.\n") # printa questo
...
def proxylist(): # funzione per la creazione della proxylist
	global proxies
	print ("\n  Đang kiểm tra các IP trùng nhau ...")
	...
	print("  Hiện tại trong danh sách có %s IP proxy" % (len(open("proxy.txt").readlines()))) # per vedere quante lines (e quindi quanti proxy) ci sono nel file
	print ("\n  Đã cập nhật danh sách proxy\n")
...
def main(): # funzione effettiva del programma.
	try:
		...
		print ("\n  Đang tải xuống từ danh sách proxy miễn phí ...")
		url = "http://free-proxy-list.net/"
		proxyget2(url) # manda url alla funzione
		url = "https://www.us-proxy.org/"
		proxyget2(url)
		print("  Hiện tại trong danh sách có %s IP proxy" % (len(open("proxy.txt").readlines()))) # printa la lunghezza attuale del file, che sarebbe il numero di proxy

		print ("\n  Đang tải xuống từ blogspot ...\n")
		...
		print("  Hiện tại trong danh sách có %s IP proxy" % (len(open("proxy.txt").readlines())))

		print ("\n  Đang tải xuống từ nhiều máy nhân bản khác nhau ...")
		for position, url in enumerate(nurls):
			proxyget(url)
			print("  Đã hoàn thành tải xuống: (%s/%s)\n  Hiện tại trong danh sách có %s IP proxy" % (position+1, len(nurls), len(open("proxy.txt").readlines())))
		...
		print("\n")
		while True:
			choice = input("\n  [+] Bạn có muốn lọc proxy còn hoạt động không [y/n] (default yes): ") # scelta di quello che vuole l'utente
...
if __name__ == '__main__':
    ...
    #+#    #+#     #+#         #+#     #+#        
    ###    ###     ###         ###     ###        
			scanner proxy by NgThanhVinh
	''')
		choice = input("\n  [+] Bạn có muốn tải xuống proxy không [y/n] (default yes): ")
